"""
CNBC Indonesia news provider.

Fetches financial headlines from CNBC Indonesia's market RSS feed.
CNBC Indonesia is a major source for IDX market commentary, analyst views,
and corporate news.
...
CNBC_MARKET_RSS_URL = "https://www.cnbcindonesia.com/market/rss"
DEFAULT_TIMEOUT = 10
MAX_TITLE_LENGTH = 500
USER_AGENT = "ai-saham/1.0"


class CNBCIndonesiaNewsProvider:
    """Fetches IDX market headlines from CNBC Indonesia RSS.

    CNBC Indonesia covers IDX stocks, market commentary, and corporate news.
    Headlines are filtered client-side: only items mentioning the ticker
    in title or description are returned.

    Usage:
        provider = CNBCIndonesiaNewsProvider()
        headlines = provider.fetch_headlines("BBCA", max_headlines=10, days=7)
    """

    @property
    def provider_name(self) -> str:
        return "cnbc_indonesia"
    ...
    def fetch_headlines(
        ...
        """Fetch CNBC Indonesia headlines mentioning the ticker.
        ...
        try:
            req = Request(CNBC_MARKET_RSS_URL, headers={"User-Agent": USER_AGENT})
            with urlopen(req, timeout=DEFAULT_TIMEOUT) as resp:
                xml_content = resp.read()
        except (URLError, HTTPError) as e:
            logger.warning("CNBC Indonesia RSS unavailable: %s", e)
            return []

        try:
            return self._parse_rss(xml_content, ticker_upper, cutoff, max_headlines)
        except ET.ParseError as e:
            logger.warning("Failed to parse CNBC Indonesia RSS: %s", e)
            return []
