"""
CLI: `saham inspect signal accum TICKER`

Read-only SignalEngine inspection for the accumulation-flow contract only
(same boundary as `screen accum` assessor). Not pre-open, not swing TradeSetup.
...
def accum(
    ...
    """Inspect SignalEngine scoring for accumulation-flow only (read-only).

    Contract: accumulation-flow (same boundary as the accumulation screen).
    Not pre-open auction signal. Not swing TradeSetup (`saham plan swing`).
    Public path: ``saham inspect signal accum``.
    """
    cfg = load_app_config()
    ...
    try:
        ...
        typer.echo(f"[error] Failed to inspect accumulation-flow signal: {exc}", err=True)
