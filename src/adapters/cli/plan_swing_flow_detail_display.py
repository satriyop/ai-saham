...
def print_flow_detail_panel(ctx: SwingOutputDisplayContext) -> None:
    ...
    if include_flow_detail and accum:
        ...
        flow_group.append(
            Text(
                (
                    "Scope: broker-flow and attribution diagnostics. SignalEngine uses the"
                    "composite foreign-flow score below, not pure foreign net flow."
                ),
                style="dim",
            )
        )
        flow_group.append(
            Text(
                (
                    "Longer-term flow context below is diagnostic only and does not directly"
                    "change Verdict."
                ),
                style="dim",
            )
        ...
        fvwap_str = (
            f"{accum.vwap_discount_pct:+.1f}%" if accum.vwap_discount_pct is not None else "—"
        )
        vwap_pct_str = f"{accum.vwap_pct:+.1f}%" if accum.vwap_pct is not None else "—"
        bb_str = (
            f"{int(accum.bb_width_pctile * 100)}%" if accum.bb_width_pctile is not None else "—"
        )
        ...
        if accum.accum_score >= config.watch_min_score and not has_current_flow_confirmation(accum):
            flow_group.append(
                Text(
                    "Note: composite score is in watch-zone, but current foreign flow is not "
                    "confirming "
        ...
        if has_bandar_distribution(bd_for_note):
            flow_group.append(
                Text(
                    "Note: Bandar detector shows distribution; RiskEngine can block execution "
                    "even when "
        ...
        if accum.rights_issue_risk:
            corp_flags.append(
                Text("⚠ RIGHTS ISSUE — dilution risk within hold window", style="yellow")
            )
        ...
        if accum.seasonal_edge is not None:
            ...
            corp_flags.append(
                Text(f"★ SEASONAL {se.label} (accum bonus {se.score:+.2f})", style=se_color)
            )
        ...
        if accum.analyst_consensus is not None:
            ...
            ac_color = (
                ...
                else ("red" if ac.sell_count > ac.buy_count else "white")
            )
        ...
        if accum.bandar_detector is not None:
            ...
            bd_color = (
                ...
                if bd.accumulation_score >= 4
                    ...
                    "yellow" if bd.is_accumulating else ("red" if bd.is_distributing else "white")
                )
            )
        ...
        if accum.fundamentals is not None:
            ...
            fund_color = (
                "green"
                if fund.is_quality
                else ("yellow" if fund.roe_ttm is not None and fund.roe_ttm >= 10.0 else "red")
            )
    ...
    if include_flow_detail and flow_detail:
        ...
        flow_group.append(
            Text(
                f"Longer-Term Flow Context ({flow_detail.window_sessions} "
                f"broker sessions, diagnostic only) through {fmt_date(flow_detail.through_date)}",
                style="bold cyan",
            )
        )
        if (
            ...
            flow_group.append(
                Text(
                    (
                        "Interpretation: recent signal-window accumulation is occurring"
                        "inside a negative longer-term flow backdrop."
                    ),
                    style="dim yellow",
                )
            )
            ...
            flow_group.append(
                Text(
                    (
                        "Interpretation: longer-term flow is negative and the current signal"
                        "window lacks foreign-flow confirmation."
                    ),
                    style="dim yellow",
                )
            )
            ...
            flow_group.append(
                Text(
                    (
                        "Interpretation: longer-term net buying exists, but the current"
                        "signal window is still weak."
                    ),
                    style="dim yellow",
                )
            )
        ...
        latest_flow = (
            ...
            if flow_detail.latest_net_flow is not None
            else "N/A"
        )
    ...
    if include_flow_detail and broker_detail:
        ...
        flow_group.append(
            Text(
                "Attribution "
                f"({broker_detail.detail_sessions}/"
                f"{broker_detail.window_sessions} sessions) "
                f"via {broker_detail.source}",
                style="bold cyan",
            )
        )
        ...
        for j in range(max_len):
            ...
            if j < len(broker_detail.top_sellers):
                ...
                sell_str = (
                    f"{s.broker_code}: {fmt_money_short(abs(s.net_value))} ({s.active_sessions}s)"
                )
        ...
        smart_share = (
            ...
            if broker_detail.smart_share_pct is not None
            else "N/A"
        )
        buyer_share = (
            f"{broker_detail.top_buyer_share_pct:.1f}%"
            if broker_detail.top_buyer_share_pct is not None
            else "N/A"
        )
        seller_share = (
            f"{broker_detail.top_seller_share_pct:.1f}%"
            if broker_detail.top_seller_share_pct is not None
            else "N/A"
        )
        ...
        metrics_table.add_row(
            "Smart Money Flow", f"{fmt_money_short_signed(broker_detail.smart_flow)} IDR"
        )
        metrics_table.add_row(
            "Noise Flow", f"{fmt_money_short_signed(broker_detail.noise_flow)} IDR"
        )
        metrics_table.add_row(
            "Weighted Net Flow", f"{fmt_money_short_signed(broker_detail.weighted_net_flow)} IDR"
        )
        metrics_table.add_row("Smart Share %", smart_share)
        metrics_table.add_row(
            "Concentration", f"Top Buyer: {buyer_share} | Top Seller: {seller_share}"
        )
        metrics_table.add_row(
            "Quality Profile", f"{broker_detail.quality} ({broker_detail.broker_weight_quality})"
        )
