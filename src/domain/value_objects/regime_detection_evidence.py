...
class RegimeDetectionEvidence:
    ...
    def to_dict(self) -> dict:
        return {
            ...
            "ihsg_20d_return": round(self.ihsg_20d_return, 4) if self.ihsg_20d_return is not None else None,
            "ihsg_trend_structure": self.ihsg_trend_structure,
            "ihsg_breadth_pct_above_ma": round(self.ihsg_breadth_pct_above_ma, 4) if self.ihsg_breadth_pct_above_ma is not None else None,
            "ihsg_volume_trend": round(self.ihsg_volume_trend, 4) if self.ihsg_volume_trend is not None else None,
            "ihsg_atr_pct": round(self.ihsg_atr_pct, 4) if self.ihsg_atr_pct is not None else None,
            "idx_foreign_flow_5d": round(self.idx_foreign_flow_5d, 2) if self.idx_foreign_flow_5d is not None else None,
            "idx_foreign_flow_20d": round(self.idx_foreign_flow_20d, 2) if self.idx_foreign_flow_20d is not None else None,
            "foreign_buy_streak": self.foreign_buy_streak,
            "foreign_sell_streak": self.foreign_sell_streak,
            "banking_sector_vs_ihsg": round(self.banking_sector_vs_ihsg, 4) if self.banking_sector_vs_ihsg is not None else None,
            "sector_breadth": round(self.sector_breadth, 4) if self.sector_breadth is not None else None,
            "forward_ihsg_return_5d": round(self.forward_ihsg_return_5d, 4) if self.forward_ihsg_return_5d is not None else None,
            "forward_ihsg_return_10d": round(self.forward_ihsg_return_10d, 4) if self.forward_ihsg_return_10d is not None else None,
            "forward_ihsg_return_20d": round(self.forward_ihsg_return_20d, 4) if self.forward_ihsg_return_20d is not None else None,
        }

    def detection_inputs_dict(self) -> dict:
        """Return only the raw detection inputs (no regime output, no forward labels) for fingerprint storage."""
        return {
            "ihsg_20d_return": round(self.ihsg_20d_return, 4) if self.ihsg_20d_return is not None else None,
            "ihsg_trend_structure": self.ihsg_trend_structure,
            "ihsg_breadth_pct_above_ma": round(self.ihsg_breadth_pct_above_ma, 4) if self.ihsg_breadth_pct_above_ma is not None else None,
