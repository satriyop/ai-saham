...
class SectorContextEvidence:
    ...
    def __post_init__(self) -> None:
        ...
        if self.sector_20d_return is not None and not (-1.0 <= self.sector_20d_return <= 5.0):
            raise ValueError(
                f"sector_20d_return {self.sector_20d_return:.4f} outside plausible range"
            )
        if self.sector_breadth is not None and not (0.0 <= self.sector_breadth <= 1.0):
            raise ValueError(f"sector_breadth must be in [0,1], got {self.sector_breadth}")
    ...
    def to_dict(self) -> dict[str, Any]:
        return {
            ...
            "sector_20d_return": (
                round(self.sector_20d_return, 6) if self.sector_20d_return is not None else None
            ),
            "sector_vs_ihsg_20d": (
                round(self.sector_vs_ihsg_20d, 6) if self.sector_vs_ihsg_20d is not None else None
            ),
            "sector_breadth": (
                round(self.sector_breadth, 4) if self.sector_breadth is not None else None
            ),
            "ticker_vs_sector_rs": (
                round(self.ticker_vs_sector_rs, 6) if self.ticker_vs_sector_rs is not None else None
            ),
