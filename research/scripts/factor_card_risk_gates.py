...
def build_report(panel: list[PanelRow], db_path: Path) -> str:
    ...
    lines.extend(
        [
            ...
            "",
            "**Not measurable on this panel yet.** All accumulation gates use",
            "`missing_data_action: skip` in `config/risk_engine.yaml`, but per-gate",
            "skip/missing outcomes are not persisted. Child `RiskAssessment` stores",
            "only the first firing gate (short-circuit), not GateContext completeness.",
            "",
            "Engineering follow-up for C2:",
            "1. Persist per-gate evaluation list + missingness flags on the risk child.",
            "2. Re-backfill schema cohort with that contract (clean break / schema bump).",
            "3. Re-run this card with fail-open vs counterfactual fail-closed cohorts.",
            "",
            ...
            "- Next: C2 GateContext persistence, or re-backfill to populate",
            "  `observation_risk_assessments` child rows for full JSON audit.",
            "",
        ]
    )
    return "\n".join(lines)
