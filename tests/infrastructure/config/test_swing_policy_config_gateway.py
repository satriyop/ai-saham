...
def test_gateway_applies_exact_allowlisted_change_and_rereads(tmp_path: Path) -> None:
    ...
    path.parent.mkdir()
    path.write_text(
        yaml.safe_dump({"signal_engine": {"classification": {"strong_min_score": 70}}})
    )
    ...
    changes = {
        "config/signal_engine.yaml:signal_engine.classification.strong_min_score": 68
    }
...
def test_gateway_reports_dirty_target(tmp_path: Path) -> None:
    ...
    assert (
        gateway.target_files_clean(
            {"config/signal_engine.yaml:signal_engine.threshold": 68}
        )
        is False
    )
