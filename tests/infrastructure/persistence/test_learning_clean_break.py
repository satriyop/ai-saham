...
def _seed_legacy_database(path) -> None:
    ...
    connection.execute(
        "INSERT INTO candidate_observations_quarantine DEFAULT VALUES"
    )
    connection.execute(
        "INSERT INTO signal_forward_labels_quarantine DEFAULT VALUES"
    )
    connection.commit()
    connection.close()
...
def test_clean_break_rolls_back_when_existing_learning_table_is_nonempty(
    ...
    def invalid_schema(connection):
        ...
        connection.execute(
            "INSERT INTO learning_observations VALUES ('unexpected')"
        )
