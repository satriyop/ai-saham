...
class TestFreeFloatGateTriggers:
    def test_thin_float_triggers_high_risk(self):
        gate = FreeFloatGate()
        result = gate.evaluate(_ctx(6.7), current_risk=RiskLevel.LOW_RISK)
    ...
    def test_float_below_threshold_triggers(self):
        ...
        result = gate.evaluate(_ctx(14.9), current_risk=RiskLevel.LOW_RISK)
        assert result.triggered
        assert result.override_risk == RiskLevel.HIGH_RISK
    ...
    def test_float_at_threshold_passes(self):
        ...
        gate = FreeFloatGate(min_free_float_pct=15.0)
        result = gate.evaluate(_ctx(15.0), current_risk=RiskLevel.LOW_RISK)
        assert not result.triggered
        assert result.override_risk is None

    def test_float_above_threshold_passes(self):
        gate = FreeFloatGate()
        result = gate.evaluate(_ctx(60.0), current_risk=RiskLevel.LOW_RISK)
        assert not result.triggered

    def test_zero_float_triggers(self):
        gate = FreeFloatGate()
        result = gate.evaluate(_ctx(0.0), current_risk=RiskLevel.LOW_RISK)
        assert result.triggered
        assert result.override_risk == RiskLevel.HIGH_RISK

    def test_hundred_pct_float_passes(self):
        gate = FreeFloatGate()
        result = gate.evaluate(_ctx(100.0), current_risk=RiskLevel.LOW_RISK)
        assert not result.triggered
...
class TestFreeFloatGateMissingData:
    def test_none_float_passes_silently(self):
        gate = FreeFloatGate()
        result = gate.evaluate(_ctx(None), current_risk=RiskLevel.LOW_RISK)
        assert not result.triggered
        assert result.override_risk is None
    ...
    def test_none_float_does_not_alter_current_risk(self):
        gate = FreeFloatGate()
        result = gate.evaluate(_ctx(None), current_risk=RiskLevel.HIGH_RISK)
        assert not result.triggered
...
class TestFreeFloatGateIgnoresCurrentRisk:
    ...
    def test_fires_when_current_risk_is_moderate(self):
        gate = FreeFloatGate()
        result = gate.evaluate(_ctx(6.7), current_risk=RiskLevel.MODERATE)
        assert result.triggered

    def test_fires_when_current_risk_is_high(self):
        gate = FreeFloatGate()
        result = gate.evaluate(_ctx(6.7), current_risk=RiskLevel.HIGH_RISK)
        assert result.triggered
...
class TestFreeFloatGateReason:
    ...
    def test_triggered_reason_contains_threshold(self):
        gate = FreeFloatGate(min_free_float_pct=15.0)
        result = gate.evaluate(_ctx(10.0), current_risk=RiskLevel.MODERATE)
        assert "15" in result.reason

    def test_pass_reason_contains_float_value(self):
        gate = FreeFloatGate()
        result = gate.evaluate(_ctx(45.0), current_risk=RiskLevel.LOW_RISK)
        assert "45.0" in result.reason

    def test_triggered_confidence_is_100(self):
        gate = FreeFloatGate()
        result = gate.evaluate(_ctx(5.0), current_risk=RiskLevel.LOW_RISK)
        assert result.confidence == 100

    def test_pass_confidence_is_100(self):
        gate = FreeFloatGate()
        result = gate.evaluate(_ctx(50.0), current_risk=RiskLevel.LOW_RISK)
        assert result.confidence == 100
...
class TestFreeFloatGateCustomThreshold:
    def test_msci_threshold_fires_at_border(self):
        gate = FreeFloatGate(min_free_float_pct=15.0)
        assert gate.evaluate(_ctx(14.9), current_risk=RiskLevel.LOW_RISK).triggered
        assert not gate.evaluate(_ctx(15.1), current_risk=RiskLevel.LOW_RISK).triggered
    ...
    def test_strict_threshold_catches_more(self):
        ...
        assert gate_strict.evaluate(_ctx(20.0), current_risk=RiskLevel.LOW_RISK).triggered
        assert not gate_default.evaluate(_ctx(20.0), current_risk=RiskLevel.LOW_RISK).triggered
