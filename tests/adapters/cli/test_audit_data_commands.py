...
def test_repair_seasonality_cache_apply_requires_explicit_db(monkeypatch):
    ...
    def _must_not_run(*args, **kwargs):
        raise AssertionError(
            "_run_repair_seasonality_cache must not be called when --db is missing"
        )
    ...
    result = runner.invoke(
        app,
        ["audit", "data", "repair-seasonality-cache", "--apply"],
    )
...
def test_repair_seasonality_cache_dry_run_without_db_is_allowed(monkeypatch):
    ...
    result = runner.invoke(
        app,
        ["audit", "data", "repair-seasonality-cache", "--format", "json"],
    )
