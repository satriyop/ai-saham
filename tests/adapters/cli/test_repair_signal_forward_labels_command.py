...
def test_repair_signal_forward_labels_apply_requires_explicit_db(monkeypatch):
    ...
    def _must_not_run(*args, **kwargs):
        raise AssertionError(
            "_run_repair_signal_forward_labels must not be called when --db is missing"
        )
    ...
    result = runner.invoke(
        app,
        ["audit", "data", "repair-signal-forward-labels", "--apply"],
    )
...
def test_repair_signal_forward_labels_dry_run_without_db_is_allowed(monkeypatch):
    ...
    result = runner.invoke(
        app,
        ["audit", "data", "repair-signal-forward-labels", "--format", "json"],
    )
