...
def test_fetch_swing_sentiment_suppresses_provider_noise_by_default(
    ...
    monkeypatch.setattr(
        "src.adapters.composition.swing_optional_fetchers.SentimentFactory.create_news_provider",
        lambda: NoisyNewsProvider(),
    )
    monkeypatch.setattr(
        "src.adapters.composition.swing_optional_fetchers.SentimentFactory.create_classifier",
