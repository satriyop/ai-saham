...
def _seed_schema3_empty_hash_observation(db_path: Path) -> None:
    from src.infrastructure.persistence.sqlite_candidate_observations_repository import (
        SQLiteCandidateObservationsRepository,
    )
...
def test_generate_rejects_schema3_empty_hash_observation_table_mode(tmp_path):
    db_path = tmp_path / "signal_labels.db"
    _seed_schema3_empty_hash_observation(db_path)
...
def test_generate_rejects_schema3_empty_hash_observation_json_mode(tmp_path):
    db_path = tmp_path / "signal_labels.db"
    _seed_schema3_empty_hash_observation(db_path)
