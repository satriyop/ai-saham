...
- Single JSON adds `applied_filters`, `counts_before_filter`, `counts_after_filter`, `data_as_of`, `universe`, `warnings`, `partial_result`. Multi JSON adds the same plus `requested_windows`/`resolved_windows`, and rows come from `ScreenAccumMultiRow` (candidates_by_window, pattern, trend, and a broker-flow field — renamed `tracked_broker_flow` in the S7 Tracked Broker Flow follow-up below) instead of raw per-window candidate dumps.
...
- **Spike status:** Complete. See `tdocs/research/s6_bci_authority_spike.md`.
  Conclusion: hypothesis supported, not confirmed (CLUSTER+negative-flow
  ...
  non-CLUSTER+negative-flow) — but this is a single-period retrospective
  backfill with a non-canonical join, not walk-forward OOS proof. Recommended
  next step: a **validation task** (walk-forward re-run + canonical-row
  re-run) before any implementation; only implement Option A (make BCI
  points conditional on positive aggregate flow direction) if that
  validation confirms the pattern. No scoring/config code has changed.
...
- [x] Canonical window (7 by default) shows Signal score, coverage %, Risk status
- [x] Tracked Broker Flow column shows data (not `n/a`) when broker_daily_flow is available
- [ ] Wall time for LQ45 multi run is materially lower than 28.48s baseline
- [x] Full test suite passes (7 pre-existing/unrelated test-order-dependent failures in `test_stock_analysis_workflow_dependencies_config_paths.py`, confirmed present before this change, per the S2/S3/S4 precedent)
- [x] `git diff --check` clean

**Resolution notes:**
- `src/application/services/screen_accum_result_projector.py` — `ScreenAccumMultiRow`/`ScreenAccumMultiProjection` gained an explicit `canonical_window` (validated to be one of the requested `windows`, else raises `ScreenAccumProjectionError`) plus `signal_score`, `signal_coverage`, `risk_status`, `setup_phase`, `data_status`, `next_action` derived only from the canonical-window candidate (`None`, never guessed from another window, when the ticker has no candidate in that window). `to_dict()` includes all new fields alongside the existing `windows`/`pattern`/`trend`/`tracked_broker_flow` keys.
- `src/application/use_case/run_accumulation_screen_workflow_use_case.py` — `_DEFAULT_MULTI_CANONICAL_WINDOW = 7`; `_execute_multi()` resolves canonical window to 7 when requested, else the first requested window, and passes it into the projector. No new CLI flag added.
- `src/adapters/cli/screen_accum_multi_display.py` — table columns changed from `# | Ticker | 7s | 30s | 90s | Pattern | Trend | Broker Flow` to `# | Ticker | 7s | 30s | 90s | Pattern | Signal | Risk | Phase | Data | Next | Tracked Broker Flow`; display only renders precomputed `ScreenAccumMultiRow` fields, no business logic in the adapter. Tracked Broker Flow now renders `label tracked/sessions` (e.g. `smart+ tracked/5s`) and `N/A` (not lowercase `n/a`) when absent. Run-context panel states the canonical window and that Signal/Risk/Phase/Data/Next come from it only.
- `src/adapters/cli/screen_accum_commands.py` — multi JSON path now builds each ticker entry from `ScreenAccumMultiRow.to_dict()` (flattening its `windows` dict back to top-level `{w}_sessions` keys for backward compatibility with existing consumers/tests) instead of manually re-deriving a parallel JSON shape.
- **Tracked Broker Flow rename (clean break, post-implementation correction):** `broker_summaries` and `broker_daily_flow` turned out to be different data products — `broker_summaries` is stock-level with true full-market foreign/turnover totals (its `top_buyers`/`top_sellers` are frequently empty in the current DB), while `broker_daily_flow` is per-day per-broker rows covering only a configured *tracked-broker* subset (see `BrokerDailyFlow`'s imbalance note). The multi-window "Broker Flow" column was built on `broker_daily_flow` but named/labeled generically, which reads as full broker composition. Renamed throughout: `src/application/services/broker_quality.py` → `src/application/services/tracked_broker_flow.py`; `BrokerQualitySnapshot` → `TrackedBrokerFlowSnapshot` (gained an explicit `scope: str = "tracked_brokers"` field, always populated); `compute_broker_quality`/`compute_broker_quality_batch` → `compute_tracked_broker_flow`/`compute_tracked_broker_flow_batch`, and reimplemented against `broker_repo.get_broker_daily_flows()` instead of `get_broker_summaries()` (session count now counts distinct trading dates across per-broker rows, not summary rows). `ScreenAccumMultiRow.broker_quality` → `tracked_broker_flow`; JSON key `broker_quality` removed with no compatibility alias, replaced by `tracked_broker_flow` (including `scope`). Table header/cell renamed to "Tracked Broker Flow" / `label tracked/Ns` — the cell literal always reads `tracked`, never the underlying data source (`idx`/`stockbit`), so it cannot be misread as full-market composition. If full top-broker composition is ever needed, it must be sourced separately from `broker_summaries.top_buyers/top_sellers` under its own name (e.g. `full_broker_composition`) — not merged into this concept. No scoring, SignalEngine, risk/setup decision, or fetch behavior changed.
- **Not done**: the expensive per-ticker enrichment/Signal/Risk/setup-phase work still runs once per requested window (`_execute_multi()` still calls `self._screen_use_case.execute()` per window) rather than once per ticker from a single shared 90-session series. `AccumulationScreenUseCase` has no window-only recompute path to reuse, and building one is a nontrivial refactor of that use case's internals (structural pruning, enrichment, SignalEngine, risk funnel all keyed to a single `window_days` request). Doing that safely is out of scope for this pass — see the `TODO(S7 follow-up)` comment in `_execute_multi()`. Only the canonical window's evidence is surfaced to the user, so the redundant work is wasted CPU, not wrong output; wall-time was not re-benchmarked against the 28.48s baseline since the redundant execution is unchanged.
- Verified: focused screen-accum + tracked-broker-flow tests pass (89 total — projector, workflow, display, JSON/save, tracked-broker-flow service, tracked-broker-flow CLI-adjacent); live smoke `saham screen accum INDF BBCA --multi` shows real Signal/Risk/Phase/Data/Next per row and `noise- tracked/5s` / `smart- tracked/5s` Tracked Broker Flow cells instead of `n/a`; JSON confirmed to include `tracked_broker_flow` (with `scope: "tracked_brokers"`) and to omit `broker_quality` entirely; single-window `saham screen accum INDF` output unchanged. Full suite: 4068 passed, only the same 7 pre-existing/unrelated failures noted above. `git diff --check` clean.
