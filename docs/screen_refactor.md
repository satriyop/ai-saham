...
1. **HIGH — `src/application/services/flow_confirmation_evidence_builder.py` had its own independent, unrescaled copy of the component weights** (`_FLOW_SIGNAL_WEIGHTS` = 40/30/20/10/15, `_FLOW_MAX_SCORE` = 115.0), never wired to `ForeignFlowScorePolicy` or caught by any grep pattern used during implementation. Because the builder normalizes the (now smaller, rescaled) breakdown scores by the old denominator, it silently *underestimated* flow strength — e.g. a candidate whose old proportional strength was 0.904 would compute to 0.753 instead of the correct 0.904. This feeds `FlowConfirmationEvidence.capped_strength`, which SignalEngine's canonical `institutional_flow` evidence group scores from — a real production scoring regression, not just cosmetic drift. **First fix (same-day patch):** rescaled the hardcoded weights to 33.3/25.0/16.7/8.3/12.5 with `_FLOW_MAX_SCORE` derived as `sum(...)` instead of a second literal. **Follow-up refactor (structural fix, next day):** the hardcoded copy was removed entirely — `FlowConfirmationEvidenceBuilder` now takes an injected `foreign_flow_score_policy: ForeignFlowScorePolicy | None` constructor param and derives its weight map (and the denominator, via `sum()`) from it directly, so it can never drift from `ScoreForeignFlowUseCase`'s policy again regardless of future tuning. `AccumulationScreenUseCase` and `SwingAnalysisWorkflowUseCase` both wire the active policy in; test-only call sites keep the default-policy constructor. See `test_flow_confirmation_evidence_builder.py` for the full test matrix (default-policy parity, custom-policy denominator, disabled-component handling, all-disabled zero-division guard).
...
## Post-refactor review round 2 (2026-07-09): 1 more finding, fixed

5. **LOW-MEDIUM — `_make_sub_signal()` still used the raw breakdown score for a policy-disabled component even though its max weight was correctly zeroed.** A policy-disabled component's weight becomes `0.0` (correct), but the *score* was still read straight from `breakdown.get(key, 0.0)` — so any breakdown data whose disabled-key score wasn't itself `0.0` (stale data, a malformed/custom `ForeignFlowEvidence`, replayed pre-policy-change records) would still leak into `flow_score_ex_bb`'s numerator even though its weight was excluded from the denominator. `ScoreForeignFlowUseCase` itself always emits `0.0` for a disabled component today, so this had no live production impact, but it defeated the "policy is the single source of truth" goal of this refactor for any non-canonical caller. **Fixed:** `_make_sub_signal()` now zeroes the score whenever `weight <= 0`, regardless of what the breakdown data says. Updated `test_disabled_component_does_not_crash_even_with_score_present` (renamed to `test_disabled_component_score_is_zeroed_even_if_breakdown_carries_a_value`) to assert score, weight, numerator, and denominator all correctly exclude the disabled component.
