...
| 3 | **Consistency** — streak (days), months | daily streak; multi-window smoothing; multi-day per-desk agg | days ✅ `AgentAccumulationFacts.consecutive_streak`; single-session multi-window `five_day/top-N accdist` surfaced by the `get_ticker_broker_flow` task; **multi-day per-desk streak/months = separate future analysis task** (no multi-day desk ranker exists) | 🟡 / 🔴 (multi-day) |
