...
| U6 | Stage layout (top→bottom) | **Chat-first:** sticky status chips (top) → **one 1fr transcript scroll** (question + answer + compact meta/tools inside scroll) → one-line hint. Answer is the primary surface; multi-line Do guides are **not** permanent chrome (chips only; details optional). Matches OpenCode/Codex-style infinite scroll. |
