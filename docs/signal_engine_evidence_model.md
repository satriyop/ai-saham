...
domestic rates maps (`bank`/`cement`/`property_dev`/`property`/`telco`/
`insurance`/`multifinance`/`auto` + residual consumer bags), and
`transportation` fuel/FX). Fingerprints use `smc_*` fields
