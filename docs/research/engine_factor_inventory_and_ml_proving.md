...
| Sector breadth bonus (RETIRED ADR-062) | ~~`sector_breadth.bonus_pts`~~ | Retired by ADR-062; never production baseline (no idx_groups wiring) | RETIRED — do not challenge as production policy |
...
| A3 | Sector breadth bonus (RETIRED ADR-062) | **Retired (ADR-062)** — historical residual only; not a production baseline | same | historical payload keys if present; no new producer fields |
