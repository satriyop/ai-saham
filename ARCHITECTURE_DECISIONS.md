...
| Market context | `MarketContextEngine` can condition signal **only when a path wires it into scoring**; primary screen/plan desks keep MCE as display-only diagnostic under Policy A (ADR-054/057); regime-adjusted risk remains preview |
...
| Market context | [029](docs/adr/ADR-029-market-context-engine-mce-third-first-class-application-service.md), [037](docs/adr/ADR-037-marketcontext-promotes-from-preview-only-to-canonical-signal-input.md), [047](docs/adr/ADR-047-scenario-adoption-seam-for-signal-risk-mce.md), [054](docs/adr/ADR-054-screen-judge-plan-structure-contract.md) Policy A, [057](docs/adr/ADR-057-evidence-diagnostic-evidence-corpus-vocabulary.md) |
...
| AI and sentiment | [002](docs/adr/ADR-002-rule-first-ai-optional-design.md), [013](docs/adr/ADR-013-ai-agent-governance.md), [014](docs/adr/ADR-014-full-ai-mode-explicit-bypass-mode-rejected.md), [015](docs/adr/ADR-015-sentiment-analysis-classification.md), [042](docs/adr/ADR-042-deterministic-champion-and-optional-model-challengers.md), [060](docs/adr/ADR-060-read-only-tui-context-agent.md), [061](docs/adr/ADR-061-closed-read-tool-orchestration-for-context-agent.md), [063](docs/adr/ADR-063-ephemeral-agent-session-and-context-budget.md), [064](docs/adr/ADR-064-ai-research-cockpit-bounded-multi-round-tools.md), [065](docs/adr/ADR-065-ai-research-cockpit-external-and-ro-data-l4.md), [066](docs/adr/ADR-066-ai-research-cockpit-multi-stage-destinations.md) |
| TUI cockpit and optional context agent | [051](docs/adr/ADR-051-tui-opencode-cockpit-clean-break.md), [060](docs/adr/ADR-060-read-only-tui-context-agent.md), [061](docs/adr/ADR-061-closed-read-tool-orchestration-for-context-agent.md), [063](docs/adr/ADR-063-ephemeral-agent-session-and-context-budget.md), [064](docs/adr/ADR-064-ai-research-cockpit-bounded-multi-round-tools.md), [065](docs/adr/ADR-065-ai-research-cockpit-external-and-ro-data-l4.md), [066](docs/adr/ADR-066-ai-research-cockpit-multi-stage-destinations.md), [040](docs/adr/ADR-040-manual-dependency-injection-and-composition-roots.md), [042](docs/adr/ADR-042-deterministic-champion-and-optional-model-challengers.md) |
...
| ADR-027 proposed `swing learn ...` / stale `trade swing` CLI | ADR-049 + live help | Use `policy accum` for guarded setup-config lifecycle; research/trade families per ADR-049; verify live `saham --help` |
...
| ADR-032 MCE preview-only signal | ADR-037 | Engine: MCE may condition signal when wired; risk adjustment remains preview |
| ADR-037 “MCE changes canonical TradeSetup when requested” on every desk | ADR-054 Policy A, ADR-057 | Screen/plan: MCE is display-only diagnostic; plan does not recompute Action via MCE; B-MCE into DecisionPolicy is a separate future task |
...
| ADR-032/033 `analyze swing` / overloaded `analyze` family | ADR-050 | Target verbs: `plan swing` (structure + shared composer), `inspect *` (lenses), `assess pre-open` (frozen confirm); `analyze` top-level retired on implementation |
| ADR-050 `plan` as sole swing action desk / `screen` discovery-only | ADR-054, ADR-067 | `screen` = judge candidates (universe + single-ticker deep); `plan` = trade structure and may carry forward screen judgment; not a second independent Action desk |
| ADR-051 AI chat non-goal | ADR-060, ADR-061, and ADR-063 | The cockpit may host one optional context assistant; Phase 1 is zero-tool, Phase 2 is a closed read-tool subset, Phase 3 is process-local ephemeral multi-turn with exact references; general chat, writes, CLI passthrough, open-ended tools, durable transcripts, and AI decision authority remain excluded |
| ADR-060/061 one-turn agent | ADR-063 | Follow-ups within one process are allowed only via application-owned ephemeral sessions, reference invalidation, P0–P4 budgets, and `ai.session_enabled`; no durable audit or write authority |
| ADR-061 single tool batch | ADR-064 | Bounded multi-round OUR tools (L3) for the AI Research Cockpit when `ai.tools_multi_round` is true; L1 one-batch remains when false; no external/model-invented tools |
| ADR-061/064 local tools only | ADR-065 | Named elevated RO data ask + `web_research` (DeepSeek path) with y/n confirm, tool-gap clues, fail-safe; master+per-capability flags default false |
| ADR-060/061 Judge-only entry | ADR-066 | Cockpit opens from multiple named stages via per-stage pure identity-validated context contracts; `ai.cockpit_multi_stage` default false; no new tools or authority; Judge path bit-identical |
| ADR-059 sector-breadth exclusion | ADR-062 | The dormant conglomerate-group breadth bonus remains outside production policy; active snapshot v4 is exact (nine rows) and still contains no breadth row; any future diagnostic is a new PIT-governed evidence contract rather than activation of the old rule |
| ADR-059 config-byte + hand-typed-version cohort identity | ADR-068 | Cohort identity becomes **measured behaviour**: a frozen deterministic probe set's output digest, plus the ADR-059 snapshot payload digest, plus the payload schema version. Raw config bytes, `SEMANTIC_ENGINE_VERSION`, `EVIDENCE_CONTRACT_VERSION`, and the write-only `config_hash` are deleted. Fixes over-forking (comments) and under-forking (unbumped code) with one mechanism; coverage enforced by branch floor + mutation testing; corpus clean break |
| Unbound diagnostic producer semantics inside Action cohorts | ADR-069 | Schema-14 observations bind four purpose-specific diagnostic identities to immutable typed producer snapshots without expanding ADR-068 Action identity; ml-saham requires explicit dual identities, exact window-7 paths, and sealed non-promotable artifact v4 |
| ADR-030/054 two-group accum evidence and plan-side judgment | ADR-067 | `setup_quality` is retired as a production evidence group (0/7,764 corpus presence, untunable by construction) together with the two-name blending code; `flow_confirmation` becomes the sole production group; board and ticker share one verdict and both may emit ENTER; `plan swing` carries forward screen's verdict instead of computing its own; setup phase/family/phase-readiness survive; screen scoring NON_SEMANTIC (golden gate), no corpus purge, observation schema 12→13 **forks compatibility values** |
| ADR-036 JWT store + silent `get()`/`None` session factory | ADR-070 | Store and HTTP client remain 036; **usable auth** is `StockbitAuthPort` (ensure / force_refresh / inspect). AuthFailure is typed; empty market is not auth. Headless auto-ensure vs headed recovery share one interface. PR1 migrates reauth, status, and pre-open track |
...
| [036](docs/adr/ADR-036-persisted-jwt-token-store-replaces-playwright-per-invocation-for-stockbit-data-fetching.md) | Stockbit token store | Accepted; caller/auth-failure contract amended by ADR-070 |
| [070](docs/adr/ADR-070-stockbit-auth-port-and-typed-failures.md) | Stockbit auth port and typed failures | Accepted; amends ADR-036 caller contract; port shipped, adapter/CLI/track migration in progress |
...
| [051](docs/adr/ADR-051-tui-opencode-cockpit-clean-break.md) | TUI OpenCode daily cockpit clean break | Accepted; Phases 0–5 implemented; supersedes multi-route research TUI UX; theme tokens enforced via `theme.py` + visual-parity tests (see bible § Visual) |
...
| [059](docs/adr/ADR-059-production-policy-snapshot-for-ml-challenges.md) | Production policy snapshots for ML challenges | Accepted; active v4/nine-row snapshot contract binds resolved decision policy |
...
| [063](docs/adr/ADR-063-ephemeral-agent-session-and-context-budget.md) | Ephemeral agent session, reference invalidation, and context budget | Accepted architecture; amends 060/061; process-local multi-turn only; runtime gated on implement task |
| [064](docs/adr/ADR-064-ai-research-cockpit-bounded-multi-round-tools.md) | AI Research Cockpit bounded multi-round OUR tools (L3) | Accepted architecture; amends 061; 3 rounds / 4 tools; `tools_multi_round` default false; runtime gated |
| [065](docs/adr/ADR-065-ai-research-cockpit-external-and-ro-data-l4.md) | AI Research Cockpit external research + RO data ask (L4) | Accepted architecture; amends 061/064; web_research + allowlisted RO SQL; y/n confirm; tool-gap clues; runtime gated |
| [066](docs/adr/ADR-066-ai-research-cockpit-multi-stage-destinations.md) | AI Research Cockpit multi-stage destinations + per-stage context contract | Accepted architecture; amends 060/061; per-stage pure identity-validated projections; `ai.cockpit_multi_stage` default false; runtime gated |
| [068](docs/adr/ADR-068-behavioral-engine-identity-for-accum-cohorts.md) | Behavioral engine identity for accum cohorts | **Accepted**; amends 059; identity = probe-output digest + snapshot payload digest + payload schema version; deletes config-byte hash, both hand-typed engine versions, and write-only `config_hash`; requires corpus clean break; coverage is a measured floor, not a proof |
| [069](docs/adr/ADR-069-purpose-specific-diagnostic-producer-identity.md) | Purpose-specific diagnostic producer identity | **Accepted / implemented**; schema-14 closed diagnostic bindings and immutable producer snapshots; explicit dual-ID read-only ml verification; no Action over-forking or diagnostic promotion |
| [067](docs/adr/ADR-067-retire-setup-quality-and-fix-judgment-authority-by-surface.md) | Retire `setup_quality`; one evidence basis for accum judgment; plan does not judge | **Accepted**; amends 030/054; deletes the two-name blending code; board and ticker share one verdict; live screen scoring NON_SEMANTIC (golden gate); no corpus purge; schema 12→13 forks compatibility; `strong_min_score` recalibration explicitly deferred |
