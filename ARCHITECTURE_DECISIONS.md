...
| ADR-061 desk-centric broker tool | ADR-067 | Adds stock-centric `get_ticker_broker_flow` (ticker→named accumulating/distributing desks + counts + consistency) over existing read-only use case; closes coverage rows 1/2/4; no new authority/provider |
...
| [067](docs/adr/ADR-067-ai-research-cockpit-ticker-broker-flow-tool.md) | AI Research Cockpit stock-centric ticker→broker flow tool | Accepted architecture; amends 061; one closed read tool `get_ticker_broker_flow` over existing read-only composition; side_effect NONE; runtime gated |
