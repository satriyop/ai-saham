...
- **Point-in-time enrichment workflow** — Signal backfill replay uses enrichment data valid as of the observation date. Run `saham fetch enrichment-history --universe lq45` regularly (weekly or daily) to build a PIT history. PIT tables (`company_fundamentals`, `shareholding_composition`, `analyst_cache`, `ticker_notation_cache`, `forward_estimates_cache`, `stock_meta`, `company_profile_cache`, `seasonality_cache`, `earnings_cache`) store one row per `(ticker, fetched_date)`. Historical observations before the first real snapshot will resolve to `UNKNOWN` for `market_cap_bucket` and other enrichment fields. The keystats endpoint provides 10 years of quarterly Net Income and Revenue backfilled into `company_fundamentals` with a 60-day conservative publication lag so historical `net_profit_margin` and `revenue_yoy_growth` are available for replay without look-ahead bias
...
| `--min-foreign-flow-score` | | config | Minimum composite foreign-flow score (0-100 soft cap) |
